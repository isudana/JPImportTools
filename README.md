...
- **Letter Generator** (`/letter-generator`) — fills in your details (name, address, email, date, plus a mobile number or TIN depending on the letter) and generates one of the three letters the Clearance Checklist links to: the Mobitel or Dialog mobile network stay confirmation request, or the IRD letter of personal use. Linked directly from those checklist items (pre-selecting the right letter) as a "Generate ✏️" option alongside the original template links. Deliberately limited to letters we fully control the content of — the actual government forms (GS-I, MTA-2) and the Affidavit stay as download-and-fill-by-hand, since they're flat PDFs with no fillable fields (auto-filling them would mean guessing fixed X/Y text positions per field, which breaks silently if the agency ever revises the form) and, for the Affidavit specifically, it also needs in-person notarization regardless of how it's filled in.
